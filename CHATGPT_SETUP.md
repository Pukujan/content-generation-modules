...
9. For each material claim, state what its evidence supports and what it leaves unproven. Cite immutable repository revisions or direct external sources near important public claims. For helper `0.4.2` and later, show concise descriptive link text and keep the complete pinned URL in its Markdown destination; never print a raw web URL in reader-facing prose. A citation establishes traceability, not truth. Keep when a claim applies (`valid_time`) separate from when this evidence record was entered (`recorded_at`).
