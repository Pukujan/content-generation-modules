...
The current helper contract is `0.4.1` (`v0.4.1`). It keeps versioned claim evidence, citation and provenance guidance, and the PCM-informed reader story. New target briefs also record a small set of evidence-backed `must_preserve` boundaries, which the validator checks against the generated README. The story order, brand, generated visuals, narrative-raster rules, prompt records, and image review requirements continue unchanged. Existing target repositories remain on their pinned contract until they deliberately migrate.
